...
/**
 * Deterministic Quick Check data generator per product id (NEW SPEC per architecture.md §5)
 */
export function getQuickCheckData(productId) {
  ...
  // 1. FIT CHECK (Indexed by size — Fashion/Footwear only)
  if (availableChecks.includes('fit')) {
    ...
    sizes.forEach((size, idx) => {
      // Small sizes are true to size; larger sizes (e.g. 4th or 5th index) get directional flags
      if (idx < 3) {
