...
export function getQuickCheckData(productId) {
  ...
  if (availableChecks.includes('looks')) {
    ...
    if (product.department === 'Beauty') {
      ...
      headline = `${attribute.charAt(0).toUpperCase() + attribute.slice(1)} looks ${degree} ${direction} than on screen`;
      ...
      headline = `${attribute.charAt(0).toUpperCase() + attribute.slice(1)} looks ${degree} ${direction} than in photos`;
