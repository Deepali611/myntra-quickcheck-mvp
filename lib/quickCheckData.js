...
/**
 * Deterministic Quick Check data generator per product id (Per quick-check-remaining-build.md Phase 10b)
 */
  ...
  if (availableChecks.includes('fit')) {
    ...
    sizes.forEach((size, idx) => {
      ...
      if (idx === sizes.length - 1) {
        // No entry for last size -> triggers "Not enough to say for {size} yet"
        return;
      }

      if (idx < 2) {
        // True to size for S/M/6/7
        fitData[size] = { status: 'true' };
      } else if (product.department === 'Footwear') {
        // Footwear specific: sizeAccuracy + width (no chest/waist)
        ...
        // Multi-piece / apparel piece-level flag
        fitData[size] = {
